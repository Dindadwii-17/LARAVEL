...
<head>
    ...
    <title>Edit E-Book - Admin Perpustakaan</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .current-file { font-size: 13px; color: #0369a1; margin-bottom: 12px; font-weight: 600; display: block; }
    </style>
</head>
<body>
    <div class="page">
        <div class="card">
            ...
            <form action="{{ route('admin.ebooks.update', $book->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="form-grid">
                    <div class="form-group full-width">
                        <label>Judul E-Book</label>
                        <input type="text" name="title" value="{{ $book->title }}" placeholder="Masukkan judul e-book" required>
                    </div>
                    <div class="form-group">
                        <label>Penulis</label>
                        <input type="text" name="author" value="{{ $book->author }}" placeholder="Nama penulis" required>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group full-width" style="padding: 20px; background: #f0f9ff; border: 2px dashed #bae6fd; border-radius: 16px;">
                        <label style="color: #0369a1;">File E-Book (Biarkan kosong jika tidak ingin mengubah)</label>
                        @if($book->file_path)
                            <span class="current-file">📄 File saat ini: {{ basename($book->file_path) }} ({{ $book->file_size }})</span>
                        @endif
                        <input type="file" name="ebook_file" accept=".pdf,.epub" style="background: transparent; border: none; padding: 10px 0;">
                        <div class="file-info">Maksimal ukuran file: 10MB</div>
                    </div>
                    <div class="form-group full-width">
                        <label>Deskripsi E-Book</label>
                        <textarea name="description" rows="4" placeholder="Ringkasan singkat e-book...">{{ $book->description }}</textarea>
                    </div>
                </div>
